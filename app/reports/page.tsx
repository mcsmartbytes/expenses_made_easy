...
export default function ReportsPage() {
  ...
  async function loadReports() {
    ...
    try {
      ...
      // Process expenses by Schedule C line
      ...
      expenses?.forEach((exp: any) => {
        ...
        report.deductible_amount += deductibleAmount;
        report.expense_count += 1;
      });

      const sortedReport = Array.from(reportMap.values()).sort((a, b) => {
        if (!a.schedule_c_line) return 1;
        if (!b.schedule_c_line) return -1;
        return a.schedule_c_line.localeCompare(b.schedule_c_line);
      });
  ...
  function exportToCSV() {
    ...
    // Expense data
    taxReport.forEach(row => {
      csvRows.push([
        row.schedule_c_line || 'N/A',
        row.line_name || 'Other',
        row.total_amount.toFixed(2),
        row.deductible_amount.toFixed(2),
    ...
    if (mileageReport && mileageReport.total_miles > 0) {
      csvRows.push('');
      csvRows.push('Mileage Deductions');
      csvRows.push('Total Miles,Total Deduction,# of Trips');
      csvRows.push([
        mileageReport.total_miles.toFixed(2),
        mileageReport.total_deduction.toFixed(2),
        mileageReport.trip_count,
      ].join(','));
    }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6 border-2 border-gray-200">
          <div className="flex flex-wrap items-end gap-4">
            ...
            <button
              onClick={exportToCSV}
              disabled={loading || taxReport.length === 0}
              className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl shadow-lg p-6 text-white">
            ...
            <p className="text-sm text-blue-100">
              Estimated tax savings: ${(totalDeductible * 0.24).toFixed(2)}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-600 to-green-700 rounded-xl shadow-lg p-6 text-white">
            ...
            <p className="text-4xl font-bold mb-1">
              {totalExpenses > 0 ? ((totalDeductible / totalExpenses) * 100).toFixed(0) : 0}%
            </p>
            ...
            <div className="bg-white rounded-xl shadow-lg mb-8 border-2 border-gray-200">
                    ...
                    {taxReport.map((row, index) => (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          ...
                          {row.deduction_percentage > 0 && row.deduction_percentage < 100 && row.deduction_percentage !== 50 && (
                  ...
                  <tfoot className="bg-gray-50 font-semibold">
                    <tr>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-green-600 font-bold">
                        ${taxReport.reduce((sum, r) => sum + r.deductible_amount, 0).toFixed(2)}
                      </td>
              ...
              {taxReport.length === 0 && (
                <div className="p-12 text-center text-gray-500">
                  <p className="text-lg mb-2">No business expenses found</p>
                  <p className="text-sm">Add business expenses to see your tax deduction report</p>
                </div>
              )}
            </div>
        ...
        <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-blue-900">
              ...
              <p>This report is generated from your expense tracking data with IRS-compliant classifications. Always consult with a licensed tax professional or CPA before filing your taxes. Keep all receipts and documentation for at least 3 years from the date you file your return.</p>
            </div>
          </div>
        </div>
      </main>
